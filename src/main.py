...
def on_submit():
    ...
    total_tests_text = customtkinter.CTkTextbox(app, height=30, width=550)
    total_tests_text.insert('end', f"Total Tests Performed: {total_tests_str}")
    total_tests_text.pack(pady=10)
    ...
    # Define the columns
    tree["columns"] = top_states.columns.tolist()
    ...
    for state_data in top_states_list:
        tree.insert("", "end", values=state_data)
    ...
    rolling_average_text = tkinter.Text(app, height=150, width=350, yscrollcommand=scrollbar.set)
    rolling_average_text.tag_configure('center', justify='center')
    rolling_average_text.insert('end', f"Rolling Average of New Results Reported:\n\n{rolling_average_str}", 'center')
    rolling_average_text.pack(side="top", fill="both", expand=True, padx=0, pady=0)
    scrollbar.config(command=rolling_average_text.yview)
